  ...
  def create
    ...
    if @message.save
      if @message.user_id == @conversation.sender_id
        ...
        @notification = @message.notifications.create(recipient_id: @message.conversation.recipient_id, sender_id: @message.conversation.sender_id)
      else
        Pusher['notifications_'+@message.conversation.sender_id.to_s].trigger('message', {
          messaging: "メッセージが届いています。:#{@message.body}"
        })
        @notification = @message.notifications.create(recipient_id: @message.conversation.sender_id, sender_id: @message.conversation.recipient_id)
      end
